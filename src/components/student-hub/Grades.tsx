...
import { getGradeColorClasses, getSubjectColor } from '@/lib/utils';
    ...
    <div className="w-full px-4 md:px-0">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {grades.map(subjectGrade => (
          ...
          >
            <div className={`h-1 w-full ${getSubjectColor(subjectGrade.subject).accent}`} />
              ...
              {subjectGrade.grades.length > 0 ? (
                <div className="flex space-x-2 flex-wrap gap-2">
                  {subjectGrade.grades.slice(0, 3).map(g => (
                    <Button key={g.id} variant="outline" size="sm" className={`rounded-md font-semibold ${getGradeColorClasses(g.score, g.maxScore).bg} ${getGradeColorClasses(g.score, g.maxScore).text} ${getGradeColorClasses(g.score, g.maxScore).border}`}>
